...
import { UploadCloud, Columns, Download, X } from "lucide-react";
...
export default function PanoramaPage() {
  ...
  const handleAction = async (action?: "share" | "download") => {
    ...
    try {
      ...
      const pWidth = img.naturalWidth / parts;
      const pHeight = img.naturalHeight;
      ...
      for (let c = 0; c < parts; c++) {
        ...
        ctx.drawImage(img, c * pWidth, 0, pWidth, pHeight, 0, 0, pWidth, pHeight);
        
        await new Promise<void>((resolve) => {
          canvas.toBlob((blob) => {
            if (blob) newFiles.push({ url: URL.createObjectURL(blob), name: `panorama_${c+1}.jpg` });
            resolve();
          }, "image/jpeg", 1.0);
        });
      }
  ...
  return (
    <div className="min-h-screen bg-[#0f0f17]" dir="rtl">
      <div className="p-4">
        <ToolHeader 
          title="بانوراما (سلايدر إنستغرام)" 
          description="تقسيم صورة عرضية إلى عدة صور طولية" 
          icon={<Columns className="w-5 h-5 text-white/80" />} 
          ...
          {!imageSrc ? (
            <div onClick={() => fileInputRef.current?.click()}
              ...
              <div className="flex flex-col items-center gap-3 py-10">
                ...
                <p className="text-white font-bold">ارفع صورة عريضة</p>
              </div>
            </div>
          ) : (
            <div className="relative w-full flex justify-center bg-black/40 rounded-2xl p-4">
              <div className="relative inline-block w-full max-w-xl overflow-hidden rounded-xl shadow-xl">
                <img ref={imgRef} src={imageSrc} className="w-full h-auto block opacity-60" />
                <div className="absolute inset-0 flex">
                  {Array.from({ length: parts }).map((_, i) => (
                    <div key={i} className="flex-1 border-r-2 border-dashed border-white/50 h-full flex items-center justify-center bg-white/5 pointer-events-none">
                      <span className="bg-black/60 text-white font-bold rounded-full w-6 h-6 flex items-center justify-center text-[10px]">{i + 1}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {imageSrc && (
          <div className="space-y-4">
            <div className="bg-white/5 rounded-2xl p-4">
              <h2 className="text-white/50 text-xs font-bold mb-3">عدد الأجزاء</h2>
              <div className="grid grid-cols-5 gap-2">
                {[2, 3, 4, 5, 6].map(num => (
                  <button key={num} onClick={() => setParts(num)}
                    className={`py-3 text-sm font-bold rounded-xl transition-all ${
                      parts === num ? "bg-orange-600 text-white shadow-lg" : "bg-white/10 text-white/60 hover:bg-white/15"
                    }`}>
                    {num}
                  </button>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <div className="bg-[#1a1a24] rounded-3xl w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-white/10">
              ...
              <div className="flex overflow-x-auto gap-4 pb-4 snap-x hide-scrollbar">
                {resultFiles.map((file, i) => (
                  <div key={i} className="shrink-0 snap-center flex flex-col items-center">
                    <span className="mb-2 font-bold text-white/40 text-[10px]">الجزء {i + 1}</span>
                    <img src={file.url} className="h-[40vh] max-w-none rounded-lg border-2 border-white/10 shadow-lg" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="p-4 border-t border-white/10 bg-white/5">
              <button onClick={handleDownloadAll} className="w-full py-3 bg-orange-600 hover:bg-orange-500 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all">
                <Download className="w-5 h-5" /> حفظ جميع الصور بالاستوديو
              </button>
            </div>
          </div>
        </div>
